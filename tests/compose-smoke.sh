...
"${compose[@]}" -p "$project" exec -T pi sh -c '\
  ...
  grep -Fqx -- "/pi_agent/pi_agent_profiles/pimatt/skills/.agents/skills" /tmp/pimatt-smoke/args; \
  grep -Fqx -- "--prompt-template" /tmp/pimatt-smoke/args; \
  grep -Fqx -- "/pi_agent/pi_agent_profiles/pimatt/skills/.pi/prompts" /tmp/pimatt-smoke/args; \
...
"${compose[@]}" -p "$project" exec -T pi sh -c '\
  ...
  grep -Fqx -- "/pi_agent/pi_agent_profiles/pibrain/skills/.agents/skills" /tmp/pibrain-smoke/args; \
